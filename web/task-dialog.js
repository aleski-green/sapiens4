...
async function refreshTaskDialog() {
  ...
  try {
    ...
    $('#task-comments-list').innerHTML=data.activity.filter(r=>r.kind==='comment').map(r=>`<article class="task-comment"><strong>${esc(['Human','Admin'].includes(r.author)?'Admin':agent(r.author).name)}</strong> <time>${esc(new Date(r.time).toLocaleString())}</time><p>${esc(r.text)}</p></article>`).join('') || '<p class="form-hint">No comments yet.</p>';
