...
final class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate, WKUIDelegate {
    ...
    let dismissUpdateButton = NSButton(title: "Dismiss", target: nil, action: nil)
    var updateBar: NSStackView!
    var workspaceTop: NSLayoutConstraint!
    var workspaceBelowBanner: NSLayoutConstraint!
    var updateNoticeKey = ""
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        updateMenu = appMenu.addItem(withTitle: "Check for Updates…", action: #selector(manualCheckUpdates), keyEquivalent: "")
        ...
        dismissUpdateButton.target = self; dismissUpdateButton.action = #selector(dismissUpdate)
        let bar = NSStackView(views: [updateLabel, updateButton, dismissUpdateButton])
        updateBar = bar; bar.isHidden = true
        ...
        workspaceTop = web.topAnchor.constraint(equalTo: content.topAnchor)
        workspaceBelowBanner = web.topAnchor.constraint(equalTo: bar.bottomAnchor)
        NSLayoutConstraint.activate([
            ...
            workspaceTop, web.bottomAnchor.constraint(equalTo: content.bottomAnchor), web.leadingAnchor.constraint(equalTo: content.leadingAnchor), web.trailingAnchor.constraint(equalTo: content.trailingAnchor)
        ])
        ...
        } else { checkServer(); updateMenu.isEnabled = false }
    }
    func runHelper(_ action: String, completion: (() -> Void)? = nil) {
        ...
        do { try process.run() } catch {
            helperRunning = false; updateMenu.isEnabled = true
            updateLabel.stringValue = error.localizedDescription
            updateButton.title = "Check for updates"; updateButton.isEnabled = true
            dismissUpdateButton.isHidden = false
            showUpdateBanner(true)
        }
    }
    func showUpdateBanner(_ visible: Bool) {
        workspaceTop.isActive = false
        workspaceBelowBanner.isActive = false
        updateBar.isHidden = !visible
        (visible ? workspaceBelowBanner : workspaceTop).isActive = true
    }
    @objc func dismissUpdate() {
        UserDefaults.standard.set(updateNoticeKey, forKey: "DismissedUpdateNotice")
        showUpdateBanner(false)
    }
    @objc func manualCheckUpdates() {
        UserDefaults.standard.removeObject(forKey: "DismissedUpdateNotice")
        showUpdateBanner(false)
        checkUpdates()
    }
    func refreshUpdateStatus() {
        ...
        updateMenu.isEnabled = !busy && !helperRunning
        updateNoticeKey = phase + ":" + ((status["sha"] as? String) ?? updateLabel.stringValue)
        let dismissed = UserDefaults.standard.string(forKey: "DismissedUpdateNotice") == updateNoticeKey
    ...
    @objc func updateClicked() {
        if updatePhase == "available" {
            ...
            dismissUpdateButton.isHidden = true
            showUpdateBanner(true)
            runHelper("update") { self.checkServer() }
        } else { manualCheckUpdates() }
    }
